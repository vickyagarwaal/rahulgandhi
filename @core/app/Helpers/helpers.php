...
function get_product_category_by_id($id,$type = ''){
    $return_val = __('uncategorized');
    $prod_cat = \App\ProductCategory::find($id);
    if (!empty($prod_cat)){
        $return_val = $prod_cat->name;
        if ($type == 'link' ){
            $return_val = '<a href="'.route('frontend.products.category',['id' => $prod_cat->id,'any' => Str::slug($prod_cat->name) ]).'">'.$prod_cat->name.'</a>';
        }
    }
...
function get_product_category_name_by_id($id){
    $return_val = __('uncategorized');

    $category_details = \App\ProductCategory::find($id);
    if (!empty($category_details)){
...
function get_wishlist_items()
{
    ...
    $return_val = !empty($old_wishlist_item) ? $old_wishlist_item : [];
...
function wishlist_total_items()
{
    $return_val = session()->get('wishlist_item');
    return !empty($return_val) ? count($return_val) : 0;
}
function render_cart_table()
{
    ...
    if (!empty($all_cart_item)) {
        ...
        foreach ($all_cart_item as $id => $item) {
            ...
            $output .= '<td><h4 class="product-title"><a href="' . route('frontend.products.single',['slug'=>$single_product->slug,'id'=>$single_product->id]) . '">' . $single_product->title . '</a></h4></td>';
        ...
        $output .= '<div class="coupon-wrap"><input type="text" class="form-control" name="coupon_code" placeholder="' . __('Coupon Code') . '"><button class="btn-medheal add_coupon_code_btn">' . __('Submit') . '</button>' . $ajax_preloader . '</div>';
        $output .= '<div class="update-cart-wrap">' . $ajax_preloader . '<button class="btn-medheal update_cart_items_btn">' . __('Update Cart') . '</button></div>';
        $output .= '</div>';
        $output .= '</td></tr>';
        $output .= "\n\t" . '</tfoot>';
...
//PRODUCT WISHLIST
function render_wishlist_table()
{
    $ajax_preloader = '<div class="ajax-loading-wrap hide">
    <i class="fas fa-spinner fa-spin mr-1"></i>
                    </div>';
    $btn_txt = get_static_option('product_add_to_cart_button_text');
    $output = '';
    $all_wishlist_item = session()->get('wishlist_item');
    if (!empty($all_wishlist_item)) {
        $output = '<div class="table-responsive cart-table"><form id="wishlist_update_form" method="post"><table class="table table-bordered">';
        $output .= "\t" . '<thead><tr>';
        $output .= "\n\t" . '<th>' . __('Serial') . '</th>';
        $output .= "\n\t" . '<th>' . __('Thumbnail') . '</th>';
        $output .= "\n\t" . '<th>' . __('Product Name') . '</th>';
        ...
        $output .= "\n\t" . '<tbody>';
        $a = 1;
        foreach ($all_wishlist_item as $id => $item) {
            ...
            $output .= '<td><div class="thumbnail">' . render_image_markup_by_attachment_id($single_product->image, '', 'thumb') . '</div></td>';
            $output .= '<td><h4 class="product-title"><a href="' . route('frontend.products.single',['slug'=>$single_product->slug,'id'=>$single_product->id]) . '">' . $single_product->title . '</a></h4></td>';
            $output .= '<td><div class="cart-action-wrap"><a href="#" class="btn btn-sm btn-danger ajax_remove_wishlist_item" data-product_id="' . $single_product->id . '"><i class="fas fa-trash-alt"></i></a>' . $ajax_preloader . '<div class="btn-wrapper"><a href="'.route('frontend.products.add.to.cart').'" class="btn btn-success btn-sm ajax_add_to_cart" data-product_id="'.$single_product->id.'" data-product_title="'.$single_product->title.'" data-product_quantity="1"><i class="fa fa-shopping-bag mr-1" aria-hidden="true"></i>'.$btn_txt.'</a></div></div></td>';
            $output .= '</tr>';
            $a++;
        }
    ...
    $output = '<div class="alert alert-warning">' . __('No Item In Wishlist!') . '</div>';
    return $output;
}
