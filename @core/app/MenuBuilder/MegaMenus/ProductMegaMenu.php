    ...
    function render($ids,$lang)
    {
        ...
        foreach ($mega_menu_items as $cat => $posts) {
            ...
            foreach ($posts as $post) {
                ...
                $output .= '<div class="thumbnail"><a href="'.route($this->route(),['slug' => $post->slug,'id' => $post->id]).'">'.render_image_markup_by_attachment_id($post->image,'','thumb').'</a></div>';
                $output .= '<div class="content">';
                $output .= '<a href="'.route($this->route(),['slug' => $post->slug,'id' => $post->id]).'"><h4 class="title">'.$post->title.'</h4></a>';
