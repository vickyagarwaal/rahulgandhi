...
class ServiceMegaMenu extends MegaMenuBase
{
    ...
    function render($ids,$lang)
    {
        ...
        $mega_menu_items = Services::whereIn('id',$ids)->get()->groupBy('category_id');
        foreach ($mega_menu_items as $cat => $posts) {
            ...
            foreach ($posts as $post) {
                $output .= '<li><a href="'.route('frontend.services.single',['slug' => \Str::slug($post->slug),'id'  =>$post->id]).'">' . $post->title . '</a></li>'."\n";
            }
            $output .= '</ul>'."\n";
            $output .= '</div>'."\n";
            $output .= '</div>'."\n";
