...
class AppointmentBookingController extends Controller
{
    ...
    public function booking_all(){
        $all_booking = AppointmentBooking::with('appointment')->orderBy('id','desc')->get();
    ...
    public function approve_payment($id){
        ...
        $subject = __('Your payment has been approved at').' '.get_static_option('site_title');
    ...
    public function reminder_mail(Request $request){
        ...
        //send mail to customer to notify his payment is approve, send mail using basic template
        $message = __('Hello').' '.$booking_details->name.'<br>';
        $message .= __('you have a pending booking id').' #'.$booking_details->id.' '.__('at').' '.get_static_option('site_title').' ';
    ...
    public function booking_view($id){
        $booking_details = AppointmentBooking::with('appointment')->findOrFail($id);
