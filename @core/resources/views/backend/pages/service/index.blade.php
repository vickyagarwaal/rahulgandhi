                                ...
                                <tbody>
                                    @foreach($all_services as $data)
                                        <tr class="{{ $data['id']}}">
                                            ...
                                            <td>{{$data->category->name ?? __('Anonymous')}}</td>
                                            <td>{{$data->sr_order}}</td>
                                            <td>{{date_format($data->created_at,'d M Y')}}</td>
                                            <td><x-status-span :status="$data->status"/></td>
                                            <td>
                                                <x-action.delete :url="route('admin.services.delete',$data->id)"/>
                                                <x-action.edit :url="route('admin.services.edit',$data->id)"/>
                                                <x-action.view :url="route('frontend.services.single',['slug'=>$data->slug,'id'=>$data->id])"/>
                                                <x-action.clone :action="route('admin.services.clone',['slug' => $data->slug, 'id' => $data->id])" :id="'$data->id'"/>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
...
<x-action.bulk.animate :url="route('admin.services.bulk.action')"/>
<x-datatable.js/>
<x-media.js/>
@endsection
