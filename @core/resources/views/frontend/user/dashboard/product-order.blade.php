        ...
        @if(count($product_orders) > 0)
            <div class="table-responsive">
                <table class="table table-striped">
                    <thead>
                    <tr>
                        <th scope="col">{{get_static_option('product_page_name')}}  {{__('Order Info')}}</th>
                    ...
                    @foreach($product_orders as $data)
                        <tr>
                            <th scope="row">
                                <div class="user-dahsboard-order-info-wrap">
                                    ...
                                    <small class="d-block"><strong>{{__('Order Status:')}}</strong>
                                        ...
                                        @elseif($data->status == 'in_transit')
                                            <span class="alert alert-info text-capitalize alert-sm alert-small">{{str_replace('_',' ',__($data->status))}}</span>
                                        @elseif($data->status == 'shipped')
                                            <span class="alert alert-custom text-capitalize alert-sm alert-small">{{str_replace('_',' ',__($data->status))}}</span>
                                        @elseif($data->status == 'complete')
                                            <span class="alert alert-custom text-capitalize alert-sm alert-small">{{str_replace('_',' ',__($data->status))}}</span>
                                        @else
                                            <span class="alert alert-custom text-capitalize alert-sm alert-small">{{__($data->status)}}</span>
                                        @endif
                                    </small>
                            ...
                            <td>
                                ...
                                @endif
                                <a href="{{route('user.dashboard.product.order.view',$data->id)}}" target="_blank" class="btn btn-sm btn-info">{{__('View Order')}}</a>
                            </td>
