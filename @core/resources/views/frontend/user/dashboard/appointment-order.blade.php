        ...
        @if(count($appointments) > 0)
            <div class="table-responsive">
                <table class="table table-striped">
                    <thead>
                    <tr>
                        <th scope="col">{{get_static_option('appointment_page_name')}} {{__('Booking Info')}}</th>
                                ...
                                <div class="user-dahsboard-order-info-wrap">
                                    <h5 class="title">
                                        @if(!empty($data->appointment))
                                            <a href="{{route('frontend.appointment.single',[$data->appointment->slug,$data->appointment->id])}}">{{$data->appointment->title}}</a>
                                        @else
                                            <div class="text-warning">{{__('This item is not available or removed')}}</div>
                                        @endif
                                    </h5>
                                    <small class="d-block"><strong>{{get_static_option('appointment_page_name')}} {{__('ID:')}}</strong> #{{$data->id}}</small>
                                ...
                                @if($data->status == 'pending')
                                    ...
                                    @if( $data->payment_gateway != 'manual_payment')
                                        <form action="{{route('frontend.appointment.booking')}}" method="post" enctype="multipart/form-data">
                                            ...
                                            <button type="submit" class="btn btn-success btn-sm margin-top-20">{{__('Pay Now')}}</button>
                                        </form>
                                    @endif
