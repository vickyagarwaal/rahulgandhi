...
<meta name="tags" content="{{$item->meta_tags}} ">
<meta name="description" content="{{$item->meta_description}}">
@endsection
@section('og-meta')
<meta name="og:title" content="{{$item->og_meta_title}}">
<meta name="og:description" content="{{$item->og_meta_description}}">
{!! render_og_meta_image_by_attachment_id($item->og_meta_image) !!}
@php
  $post_img = null;
                    ...
                    <div class="appointment-details-item">
                       <div class="top-part">
                           ...
                           <div class="content">
                               ...
                               <div class="short-description">{!! purify_html_raw($item->short_description) !!}</div>
                               @if($item->location)
                               <div class="location"><i class="fas fa-map-marked-alt"></i> {{$item->location}}</div>
                               @endif
                               ...
                               <div class="social-share-wrap">
                                   <ul class="social-share">
                                       <li>{{__('Share')}}:</li>
                                       {!! single_post_share(route('frontend.appointment.single',['slug' => $item->slug, 'id' => $item->id]),$item->title,get_attachment_url_by_id($item->image,'full')) !!}
                                   </ul>
                               </div>
                           </div>
                       </div>
                        <div class="bottom-part">
                            ...
                            <nav>
                                <div class="nav nav-tabs" role="tablist">
                                    ...
                                    <a class="nav-link"  data-toggle="tab" href="#nav-feedback" role="tab"  aria-selected="false">{{get_static_option('appointment_single_feedback_tab_title')}}</a>
                                </div>
                            </nav>
                            <div class="tab-content" >
                                <div class="tab-pane fade" id="nav-information" role="tabpanel" >
                                    <div class="information-area-wrap">
                                        ...
                                        @if(!empty(current($item->experience_info)))
                                        <div class="education-info">
                                            <h3 class="title">{{get_static_option('appointment_single_appointment_booking_educational_info_title')}}</h3>
                                            <ul class="circle-list">
                                                @foreach($item->experience_info as $info)
                                                    <li>{{$info}}</li>
                                                @endforeach
                                            </ul>
                                        </div>
                                        @endif
                                        @if(!empty(current($item->additional_info)))
                                        ...
                                        @endif
                                        @if(!empty(current($item->specialized_info)))
                                        <div class="specialised-info">
                                            ...
                                            <ul class="circle-list">
                                                @foreach($item->specialized_info as $info)
                                                    <li>{{$info}}</li>
                                                @endforeach
                                            </ul>
                                        </div>
                                 ...
                                 <div class="tab-pane fade show active" id="nav-booking" role="tabpanel" >
                                     ...
                                     <input type="hidden" id="appointment_id" name="appointment_id" value="{{$item->id}}">
                                                         ...
                                                         <div class="tab-content" >
                                                             @if(!auth()->guard('web')->check())
                                                                 <div class="tab-pane fade show active" id="nav-home" role="tabpanel">
                                                                     ...
                                                                     @if(!auth()->guard('web')->check())
                                                                         <div class="next-step">
                                                                             <button class="next-step-btn btn-medheal" style="display: none" type="button">{{__('Next Step')}}</button>
                                                                         </div>
                                                                     @endif
                                                                 </div>
                                                             @endif
                                 ...
                                 <div class="tab-pane fade" id="nav-feedback" role="tabpanel" >
                                     <div class="feedback-wrapper">
                                         ...
                                         <div class="feedback-comment-list-wrap margin-top-40">
                                             <h3 class="title">{{get_static_option('appointment_single_appointment_booking_client_feedback_title')}}</h3>
